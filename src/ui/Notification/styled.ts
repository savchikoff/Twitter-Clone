...
export const NotificationWrapper = styled.div<{ $isError: boolean }>`
  background: ${({ $isError, theme }) => ($isError ? `${theme.themeType.colors.lightPink}` : `${theme.themeType.colors.lightGreen}`)};
  padding: ${s16};
  min-width: 250px;
  border-radius: ${s8};
  position: fixed;
  ...
  top: ${s32};
  right: ${s24};
  border: ${s1} solid ${({ $isError, theme }) => ($isError ? `${theme.themeType.colors.begonia}` : `${theme.themeType.colors.green}`)};
...
export const NotificationTitle = styled.h3<{ $isError: boolean }>`
  color: ${({ $isError, theme }) => ($isError ? `${theme.themeType.colors.coralRed}` : `${theme.themeType.colors.darkGreen}`)};
  margin: ${s0};
`;

export const NotificationContent = styled.p<{ $isError: boolean }>`
  color: ${({ $isError, theme }) => ($isError ? `${theme.themeType.colors.sunsetOrange}` : `${theme.themeType.colors.seaGreen}`)};
  margin: ${s0};
  font-size: ${s16};
