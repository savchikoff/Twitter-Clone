...
import { UsersContainer, UsersHeader, UsersWrapper } from './styled';
...
const UserRecommendations: FC<IRecommendedUsersProps> = ({ searchValue }) => {
	...
	const filteredTweets = allTweets.filter((tweet) =>
		tweet.text.toLocaleLowerCase().includes((searchValue || '').toLowerCase())
	);
		...
		<UsersContainer>
			<UsersWrapper>
				...
				{isProfile
					? filteredTweets
							.filter((_, i) => i < 2)
							.map(({ tweetId, text, name, userName, createdAt }) => {
